...
RUL_MODEL = joblib.load("src_code/ML_part/saved_models/rul_model.pkl")

FAILURE_MODEL = joblib.load("src_code/ML_part/saved_models/failure_probability_model.pkl")

HEALTH_MODEL = joblib.load("src_code/ML_part/saved_models/component_health_model.pkl")



def calculate_driving_range(soc: float, battery_capacity: float, power_consumption: float):
    ...
    estimated_range = ( available_energy / power_consumption )*100
...
def calculate_charging_time(soc: float, battery_capacity: float):
    charger_power = 7.2
...
def get_predictions(vehicle_id: int,db: Session): 

    records = db.query(EVDataModel).filter(EVDataModel.vehicle_id == vehicle_id).order_by(EVDataModel.timestamp.desc()).limit(20).all()
    vehicle = db.query(VehicleModel).filter(VehicleModel.vehicle_id == vehicle_id).first()


    if not records:
        raise ValueError("No EV data found")
    ...
    feature_df = pd.DataFrame([df.mean()])
    soc = feature_df["soc"].iloc[0]
    power_consumption = feature_df["power_consumption"].iloc[0]


    rul = float(RUL_MODEL.predict(feature_df)[0])
    failure_probability = float(FAILURE_MODEL.predict_proba(feature_df)[0][1])
    health_score = float(HEALTH_MODEL.predict(feature_df)[0])

    estimated_range = calculate_driving_range(
            soc=soc, 
            battery_capacity=vehicle.battery_capacity,
            power_consumption=power_consumption
        )

    estimated_charging_time = calculate_charging_time(
            soc=soc,
            battery_capacity=vehicle.battery_capacity
        )
