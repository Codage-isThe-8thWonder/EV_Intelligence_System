...
def get_summary_card(vehicle_id: int,db: Session):
    ...
    summary = (
        db.query(
            ...
            func.max(EVDataModel.battery_temperature).label("max_battery_temperature"),

            func.avg(EVDataModel.driving_speed).label("avg_driving_speed"),

            func.sum(EVDataModel.distance_traveled).label("total_distance"),

            func.avg(EVDataModel.power_consumption).label("avg_power_consumption")

        ).filter(EVDataModel.vehicle_id == vehicle_id).first()
    )
    ...
    return {
        ...
        "max_battery_temperature": summary.max_battery_temperature,
        "avg_driving_speed": summary.avg_driving_speed,
        "total_distance": summary.total_distance,
        "avg_power_consumption" : summary.avg_power_consumption
    }
...
def get_operational_trends(vehicle_id: int, db: Session):

    records = db.query(EVDataModel).filter(EVDataModel.vehicle_id == vehicle_id).order_by(EVDataModel.timestamp).all()
...
def get_battery_analytics(vehicle_id: int, db: Session):
    records = db.query(EVDataModel).filter(EVDataModel.vehicle_id == vehicle_id).order_by(EVDataModel.timestamp).all()
...
def get_vehicle_performance(vehicle_id: int,db: Session):
