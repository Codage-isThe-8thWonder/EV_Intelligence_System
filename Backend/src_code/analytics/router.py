...
def get_operational_trends(vehicle_id: int, db: Session = Depends(get_db)):
...
def get_battery_analytics(vehicle_id: int,db: Session = Depends(get_db)):
...
def get_vehicle_performance(vehicle_id: int,db: Session = Depends(get_db)):
